...
case $DATASET in
  ...
  tinyimagenet)
    case $MODEL in
      ...
      ;;
      convnext_tiny)
        python main.py +dataset=tinyimagenet experiment=base +training.resize_dimensions=256 +model=convnext_tiny optimizer=sgd_momentum +training=tinyimagenet hydra.run.dir='./results/training/tinyimagenet/resnet50/' +attacks=imagenet training.device="$DEVICE"
      ;;
      *)
      echo -n "Unrecognized model"
    esac
  ...
  imagenet)
    case $MODEL in
      resnext50_32x4d)
        python main.py +dataset=imagenet experiment=base +model=resnext50_32x4d optimizer=sgd_momentum +training=tinyimagenet hydra.run.dir='./results/training/imagenet/resnext50_32x4d/' +attacks=imagenet training.device="$DEVICE"
      ;;
      convnext_tiny)
        python main.py +dataset=imagenet experiment=base +model=convnext_tiny optimizer=sgd_momentum +training=tinyimagenet hydra.run.dir='./results/training/imagenet/resnet50/' +attacks=imagenet training.device="$DEVICE"
      ;;
      *)
      echo -n "Unrecognized model"
    esac
